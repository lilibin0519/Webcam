package com.libin.mylibrary.db;
...
import com.libin.mylibrary.db.utils.ClassUtils;
import com.libin.mylibrary.db.utils.KeyValue;
import com.libin.mylibrary.db.utils.ManyToOne;
import com.libin.mylibrary.db.utils.OneToMany;
import com.libin.mylibrary.db.utils.TableInfo;
...
public class NiceDB {
    ...
    /** 用于保存所有数据库 */
    private static HashMap<String, NiceDB> daoMap = new HashMap<String, NiceDB>();

    private SQLiteDatabase db;
    private final DaoConfig config;

    private NiceDB(DaoConfig config) {
        if (config == null) {
    ...
    private synchronized static NiceDB getInstance(DaoConfig daoConfig) {
    	NiceDB dao = daoMap.get(daoConfig.getDbName());
        if (dao == null) {
            dao = new NiceDB(daoConfig);
     ...
     */
    public static NiceDB create(boolean isDebug) {
        Context cxt = BaseAppManager.getInstance().getTopActivity();
        if (cxt == null) {
     ...
     * @param context
     */
    public static NiceDB create(Context context) {
     ...
     *            是否是debug模式（debug模式进行数据库操作的时候将会打印sql语句）
     */
    public static NiceDB create(Context context, boolean isDebug) {
        DaoConfig config = new DaoConfig();
        config.setContext(context);
     ...
     *            数据库名称
     */
    public static NiceDB create(Context context, String dbName) {
        DaoConfig config = new DaoConfig();
        config.setContext(context);
     ...
     */
    public static NiceDB create(Context context, String dbName, boolean isDebug) {
        DaoConfig config = new DaoConfig();
        config.setContext(context);
        config.setDbName(dbName);
        config.setDebug(isDebug);
     ...
     *            数据库名称
     */
    public static NiceDB create(Context context, String targetDirectory,
            String dbName) {
        DaoConfig config = new DaoConfig();
        config.setContext(context);
        config.setDbName(dbName);
     ...
     * @return
     */
    public static NiceDB create(DaoConfig daoConfig) {
        ...
        if (cursor != null) {
            while (cursor.moveToNext()) {
                ...
                try {
                    ...
                    Log.d("nicedb",getClass().getName() + e.getMessage());
                }
            }
        }
        if (cursor != null) {
     ...
     * 删除一个数据表
     */
    public void dropDbByClassName(Class<?> clazz) {
        Cursor cursor = db.rawQuery(
                "SELECT name FROM sqlite_master WHERE type ='table'", null);
        if (cursor != null) {
            while (cursor.moveToNext()) {
                // 添加异常捕获.忽略删除所有表时出现的异常:
                // table sqlite_sequence may not be dropped
                if (!ClassUtils.getTableName(clazz).equals(cursor.getString(0))) {
                    continue;
                }
                try {
                    db.execSQL("DROP TABLE " + cursor.getString(0));
                    Trace.e("delete table : " + cursor.getString(0));
                    break;
                } catch (SQLException e) {
                    Log.d("nicedb",getClass().getName() + e.getMessage());
                }
    ...
    private void exeSqlInfo(SqlInfo sqlInfo) {
        if (sqlInfo != null) {
            ...
            Log.d("nicedb",getClass().getName() + "sava error:sqlInfo is null");
        }
    }
    ...
    private void checkTableExist(Class<?> clazz) {
        if (!tableIsExist(TableInfo.get(clazz))) {
            Trace.e("create table sql : " + SqlBuilder.getCreatTableSQL(clazz));
