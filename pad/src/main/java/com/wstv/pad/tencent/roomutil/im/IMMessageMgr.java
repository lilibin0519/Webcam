package com.wstv.pad.tencent.roomutil.im;
...
public class IMMessageMgr implements TIMMessageListener {
     ...
     * @param callback
     */
    public void initialize(final String userID, final String userSig, final int appID, final String accType, final Callback callback){
        ...
        this.mHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                mTIMSdkConfig = new TIMSdkConfig(appID)
                        .setAccoutType(accType)          // 您可以从 IM 控制台获取 accountType 信息。目前仅支持独立模式，即36862
...
//            mTIMSdkConfig.setConnectionListener(null);
    ...
    private void login(final Callback cb){
        ...
        userConfig.setUserStatusListener(new TIMUserStatusListener() {
                    ...
                    public void onForceOffline() {
                        //被其他终端踢下线
                        Trace.i("onForceOffline");
                    }

                    @Override
                    public void onUserSigExpired() {
                        //用户签名过期了，需要刷新 userSig 重新登录 SDK
                        Trace.i("onUserSigExpired");
                    }
                }).setConnectionListener(new TIMConnListener() {//设置连接状态事件监听器
                    @Override
                    ...
                    public void onDisconnected(int code, String desc) {
                        Trace.i("onDisconnected");
                    }

                    @Override
                    public void onWifiNeedAuth(String name) {
                        Trace.i("onWifiNeedAuth");
                    }
                }).setGroupEventListener(new TIMGroupEventListener() {//设置群组事件监听器
                    @Override
                    public void onGroupTipsEvent(TIMGroupTipsElem elem) {
                        Trace.i("onGroupTipsEvent, type: " + elem.getTipsType());
                    }
                }).setRefreshListener(new TIMRefreshListener() {//设置会话刷新监听器
                    @Override
                    ...
                    public void onRefreshConversation(List<TIMConversation> conversations) {
                        Trace.i("onRefreshConversation, conversation size: " + conversations.size());
                    }
                });
     ...
     */
    private class IMMessageConnCallback implements TIMCallBack {
