package com.wstv.pad.tencent.liveroom;
...
import com.wstv.pad.tencent.roomutil.commondef.RoomInfo;
import com.wstv.pad.tencent.roomutil.http.HttpRequests;
import com.wstv.pad.tencent.roomutil.http.HttpResponse;
import com.wstv.pad.tencent.roomutil.im.IMMessageMgr;
...
//import com.wstv.pad.util.RealmUtils;
...
//    public static Realm realm;
     ...
     * LiveRoom 直播房间
     */
    public LiveRoom(WsApp wsApp) {
        super(wsApp.getApplicationContext());

        mHttpRequest = new HttpRequests(BaseRoom.ROOM_SERVICE_DOMAIN+"live_room");
        ...
        mTXLivePlayer = new TXLivePlayer(wsApp.getApplicationContext());
...
//        mTXLivePlayConfig.setMinAutoAdjustCacheTime(2.0f);
        mTXLivePlayConfig.setMaxAutoAdjustCacheTime(5.0f);
...
//        realm = RealmUtils.getInstance(mContext).getRealm();
    }
     ...
     * @param cb        进入房间完成的回调
     */
    public void enterRoom(@NonNull final String roomID, @NonNull final String mixedPlayUrl, final String gender, final int level, @NonNull final TXCloudVideoView videoView, final EnterRoomCallback cb) {
...
//                    userInfo = "";
//                }
//                mHttpRequest.addAudience(roomID, wsApp.getUserBean().userId, userInfo, null);
//            }
//            callback.onSuccess();
//        }
//        else {
...
//                        String mixedPlayUrl = getMixedPlayUrlByRoomID(roomID);
                        if (mixedPlayUrl != null && mixedPlayUrl.length() > 0) {
                            ...
                            if (mHttpRequest != null) {
                                ...
                                mHttpRequest.addAudience(roomID, wsApp.getUserBean().userId, userInfo, new HttpRequests.OnResponseCallback<HttpResponse>() {
                                    @Override
...
//                            callback.onSuccess();
    ...
    public void loginIm(LoginCallback callback){
        final MainCallback cb = new MainCallback<LoginCallback, String>(callback);
        Trace.e("in loginIm");
        mIMMessageMgr.initialize(mSelfAccountInfo.userID, mSelfAccountInfo.userSig, (int)mSelfAccountInfo.sdkAppID, mSelfAccountInfo.accType, new IMMessageMgr.Callback() {
            @Override
            public void onError(int code, String errInfo) {
                mRoomListenerCallback.printLog("[LiveRoom] 登录失败: %s(%d)", errInfo, code);
                cb.onError(code, errInfo);
    ...
    public void exitRoom(final ExitRoomCallback callback) {
        ...
        mHttpRequest.delPusher(mCurrRoomID, null == mSelfAccountInfo ? PreferenceUtil.readString(AppConstant.KEY_PARAM_USER_ID) : mSelfAccountInfo.userID, new HttpRequests.OnResponseCallback<HttpResponse>() {
            @Override
            public void onResponse(int retcode, @Nullable String retmsg, @Nullable HttpResponse data) {
        ...
        if (mHttpRequest != null) {
            mHttpRequest.delAudience(mCurrRoomID, null == mSelfAccountInfo ? PreferenceUtil.readString(AppConstant.KEY_PARAM_USER_ID) : mSelfAccountInfo.userID, null);
        }
    ...
    public interface GetAudienceListCallback{
        void onError(int errCode, String errInfo);
        void onSuccess(ArrayList<Audience> audienceList);
    }
    /**
     * 获取房间列表，分页获取
        ...
        mHttpRequest.getAudienceList(roomID, new HttpRequests.OnResponseCallback<HttpResponse.AudienceList>() {
            ...
            public void onResponse(final int retcode, @Nullable final String retmsg, @Nullable HttpResponse.AudienceList data) {
                if (retcode != HttpResponse.CODE_OK || data == null || data.audiences == null){
                    ...
                    final ArrayList<Audience> arrayList = new ArrayList<>(data.audiences.size());
...
//                    }
     ...
     * @param cb
     */
    public void joinPusher(/*String acceleratePlayUrl, */final JoinPusherCallback cb) {
        if (mCurrRoomID == null || mCurrRoomID.length() == 0) {
...
//        String acceleratePlayUrl = getPushUrlByRoomID(mCurrRoomID);
        Trace.e("acceleratePlayUrl : " + acceleratePlayUrl);
...
//                    mRoomListenerCallback.onGetPusherList(newPushers);
     ...
     * @param cb
     */
    public void quitPusher(/*String mixedPlayUrl, */final QuitPusherCallback cb) {
     ...
     * @param callback
     */
    public void requestJoinPusher(String performerId, int timeout, final @NonNull RequestJoinPusherCallback callback) {
        try {
...
//            String toUserID = getRoomCreator(mCurrRoomID);
    ...
    public void getOnlinePusherList(final GetOnlinePusherListCallback callback) {
        getRoomList(0, 1000, new GetRoomListCallback() {
            @Override
     ...
     * @param msg 消息内容
     */
//    public void postPrivateMessage(final String userID, String userName, String msg, final onPrivateMessageCallback callback){
//        CommonJson<PrivateMessage> response = new CommonJson<>();
//        response.cmd = "privateMessage";
//        response.data = new PrivateMessage();
//        response.data.type = 1;
//        response.data.message= msg;
//        response.data.userAvatar = mSelfAccountInfo.userAvatar;
//        response.data.userName = mSelfAccountInfo.userName;
//        response.data.userID = mSelfAccountInfo.userID;
//        response.data.createTime = System.currentTimeMillis();
//
//        final PrivateMessage message = response.data;
//        message.with = userID;
//        message.withName = userName;
//
//        final CommonJson<PrivateMessage> finalResponse = response;
//        savePrivate(message, new Realm.Transaction.OnSuccess() {
//            @Override
//            public void onSuccess() {
//                //操作成功
//                callback.onSending(message);
//                sendPrivate(finalResponse, userID, callback);
//            }
//        }, new Realm.Transaction.OnError() {
//            @Override
//            public void onError(Throwable error) {
//                error.printStackTrace();
//                callback.onError(-99, "private message insert failed");
//            }
//        });
//    }
     ...
     * @param userID 主播ID
     */
//    public void sendPrivate(final CommonJson<PrivateMessage> response, String userID, final onPrivateMessageCallback callback) {
//        try {
//            String content = new Gson().toJson(response, new TypeToken<CommonJson<PrivateMessage>>(){}.getType());
//            mIMMessageMgr.sendC2CCustomMessage(userID, content, new IMMessageMgr.Callback() {
//                @Override
//                public void onError(final int code, final String errInfo) {
//                    callback.onError(code, errInfo);
...
//    public void postPrivatePhoto(final String userID, String userName, String url, final onPrivateMessageCallback callback){
//        CommonJson<PrivateMessage> response = new CommonJson<>();
//        response.cmd = "privateMessage";
//        response.data = new PrivateMessage();
...
//    public void savePrivate(final PrivateMessage message, Realm.Transaction.OnSuccess onSuccess, Realm.Transaction.OnError onError){
//        try {
//            realm.executeTransactionAsync(new Realm.Transaction() {
//                @Override
//                public void execute(Realm realm) {
//                    RealmResults<PrivateMessage> msgList = realm.where(PrivateMessage.class).findAllSorted("id", Sort.DESCENDING);
//                    message.id = msgList == null || msgList.size() == 0 ? 1 : msgList.get(0).id + 1;
//                    realm.copyToRealmOrUpdate(message);
////                    /*PrivateMessage privateMessage = */realm.createObject(PrivateMessage.class, message.id);
//                    //会自动更新表
////                    privateMessage.type = message.type;
////                    privateMessage.message = message.message;
////                    privateMessage.userAvatar = message.userAvatar;
////                    privateMessage.userName = message.userName;
////                    privateMessage.userID = message.userID;
////                    privateMessage.with = message.with;
////                    privateMessage.createTime = message.createTime;
////                    ChatRoom chatRoom = new ChatRoom();
////                    chatRoom.
//                    ChatRoom chatRoom = realm.where(ChatRoom.class).equalTo("userID", message.with).findFirst();
//                    if (chatRoom == null) {
//                        chatRoom = new ChatRoom();
//                        RealmResults<ChatRoom> list = realm.where(ChatRoom.class).findAllSorted("id", Sort.DESCENDING);
//                        chatRoom.id = list == null || list.size() == 0 ? 1 : list.get(0).id + 1;
////                        realm.createObject(ChatRoom.class);
//                        chatRoom.userAvatar = message.userAvatar;
//                        chatRoom.userID = message.with;
//                    }
//                    chatRoom.createTime = System.currentTimeMillis();
//                    chatRoom.message = message.message;
//                    chatRoom.type = message.type;
//                    chatRoom.userName = message.withName;
//                    realm.copyToRealmOrUpdate(chatRoom);
//                    runOnUiThread(new Runnable() {
//                        @Override
//                        public void run() {
//                            EventBus.getDefault().post(new EventCenter(2049));
//                        }
//                    });
//                }
//            }, onSuccess, onError);
...
//    }
//
//    public void readPrivate(final String userID, final long time, @NonNull RealmChangeListener<RealmResults<PrivateMessage>> listener){
//        if (realm.isClosed()) {
//            realm = Realm.getDefaultInstance();
//        }
//        RealmQuery<PrivateMessage> query = realm.where(PrivateMessage.class).equalTo("with", userID);
//        if (time != -1) {
//            query.lessThan("createTime", time);
//        }
//        Trace.e("query PrivateMessage");
//        RealmResults<PrivateMessage> results = query.findAllSortedAsync("createTime", Sort.ASCENDING);
//        results.addChangeListener(listener);
//    }
//
//    public void readChatRoom(@NonNull RealmChangeListener<RealmResults<ChatRoom>> listener){
//
//        RealmResults<ChatRoom> results = realm.where(ChatRoom.class).findAllSortedAsync("createTime", Sort.DESCENDING);
//        results.addChangeListener(listener);
//    }

//    public interface onPrivateMessageCallback{
//        void onSending(PrivateMessage message);
//
...
//    }
//
//    public interface RealmResultListListener<T extends RealmObject>{
...
//    }
    ...
    public void onPusherChanged() {
        ...
        Trace.e("onPusherChanged");
        if (mBackground == false) {
            Trace.e("onPusherChanged mSelfRoleType : " + mSelfRoleType);
            Trace.e("onPusherChanged mJoinPusher : " + mJoinPusher);
                    ...
                    public void onUpdatePushersComplete(int retcode, List<PusherInfo> newPushers, List<PusherInfo> delPushers, HashMap<String, PusherInfo> mergedPushers) {
                        if (retcode == 0) {
                            ...
                            for (PusherInfo member : delPushers) {
                                Trace.e("onPusherChanged delPushers : " + member.userID);
                                mRoomListenerCallback.onPusherQuit(member);
                                if (mSelfRoleType == LIVEROOM_ROLE_PUSHER || mJoinPusher) {
                                    Trace.e("onPusherChanged delPushers : delSubVideoStream");
                                    mStreamMixturer.delSubVideoStream(member.accelerateURL);
                                }
                            }

                            for (PusherInfo member : newPushers) {
                                Trace.e("onPusherChanged newPushers : " + member.userID);
                                mRoomListenerCallback.onPusherJoin(member);
                                if (mSelfRoleType == LIVEROOM_ROLE_PUSHER) {
                                    Trace.e("onPusherChanged newPushers : addSubVideoStream");
    ...
    public void onC2CCustomMessage(String sendId, String cmd, String message) {
        if (cmd.equalsIgnoreCase("linkmic")) {
            ...
            onChatMessageReceive(GsonUtils.gsonToBean(message, PrivateMessage.class));
        }*/
    }

//    private void onChatMessageReceive(final PrivateMessage privateMessage) {
//        privateMessage.createTime = System.currentTimeMillis();
//        privateMessage.with = privateMessage.userID;
//        privateMessage.success = true;
//        privateMessage.withName = privateMessage.userName;
//        savePrivate(privateMessage, new Realm.Transaction.OnSuccess() {
//            @Override
//            public void onSuccess() {
//                EventBus.getDefault().post(new EventCenter(2048, privateMessage));
//            }
//        }, new Realm.Transaction.OnError() {
//            @Override
//            public void onError(Throwable error) {
//                Trace.e("private message insert failed");
//            }
//        });
//    }
    ...
    public void setSelfAccountInfo(String userID, String userName, String headPicUrl, String userSig, String accType, long sdkAppID){
        this.mSelfAccountInfo = new SelfAccountInfo(userID, userName, headPicUrl, userSig, accType, sdkAppID);
        mHttpRequest.setUserID(userID);
        mAppID = sdkAppID;
    }
    ...
    public void setSelfAccountInfo(SelfAccountInfo selfAccountInfo){
        ...
        mHttpRequest.setUserID(selfAccountInfo.userID);
        mAppID = selfAccountInfo.sdkAppID;
    }
    ...
    public void setToken(String token){
        mHttpRequest.setToken(token);
    }

    public String getToken(){
        return mHttpRequest.getToken();
    }

    public void addRoom(Room room){
        mRoomList.add(new RoomInfo(room.roomID, room.roomInfo, room.roomName, room.roomCreator, room.mixedPlayURL, room.pushers));
    }

    public List<PusherInfo> getCurrentRoomPushers(){
        for (RoomInfo info : mRoomList) {
            if (info.roomID.equals(mCurrRoomID)) {
                return info.pushers;
            }
        }
        return null;
