...
public class HallFragment extends BaseFragment implements SwipeRecyclerView.OnLoadListener {
    ...
    private List<RoomInfo> adviceList;
        ...
        liveRoom.getRoomList(currentPage, AppConstant.DEFAULT_PAGE_SIZE, new IMLVBLiveRoomListener.GetRoomListCallback() {
            ...
            public void onSuccess(ArrayList<RoomInfo> roomInfoList) {
                if (currentPage == 0) {
                    data.clear();
                    adviceList.clear();
                    int size = null == roomInfoList ? 0 : roomInfoList.size();
                    for (int i = 0; i < (size > 3 ? 3 : size); i++) {
                        adviceList.add(roomInfoList.remove(0));
                    }
                    for (int i = 0; i < 3; i++) {
                        if (i < adviceList.size()) {
                            GlideLoadUtils.getInstance().glideLoad(getContext(), adviceList.get(i).pushers.get(0).userAvatar,
                                    viewList.get(i).avatar);
                            viewList.get(i).name.setText(adviceList.get(i).roomInfo);
                            viewList.get(i).times.setText(String.valueOf(adviceList.get(i).audienceCount));
                        } else {
