...
//                    if (((CamDetailActivity) context).giftSelectPosition == -1) {
//                        ToastUtils.showToastGravityCenter("请选择礼物");
//                        return;
//                    }
//                    sendListener.onSendGift(count.getText().toString(), data.get(((CamDetailActivity) context).typeIndex).giftDTOList.get(((CamDetailActivity) context).giftSelectPosition));
//                }
//            }
//        }));
//        contentView.findViewById(R.id.dialog_gift_list_send).setVisibility(((CamDetailActivity) context).giftSelectPosition == -1 ? View.INVISIBLE : View.VISIBLE);
        contentView.findViewById(R.id.dialog_gift_list_get_with).setOnClickListener(new ClickProxy(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                ((CamDetailActivity) context).showGuardBuy();
                dismiss();
            }
        }));
...
//                    return;
//                }
//                if ("1".equals(count.getText()) && !"0".equals(data.get(((CamDetailActivity) context).giftSelectPosition).ifMoreThanOnce) && !TextUtils.isEmpty(data.get(((CamDetailActivity) context).giftSelectPosition).fiveTimesUrl)) {
//                    count.setText("5");
//                    plus.setVisibility(!TextUtils.isEmpty(data.get(((CamDetailActivity) context).giftSelectPosition).tenTimesUrl) ? View.VISIBLE : View.INVISIBLE);
//                } else if ("5".equals(count.getText()) && !TextUtils.isEmpty(data.get(((CamDetailActivity) context).giftSelectPosition).tenTimesUrl)) {
//                    count.setText("10");
...
//        }));
        ...
        this.typeData = data;
        this.data = null == typeData || typeData.size() == 0 ? new ArrayList<GiftBean>() : typeData.get(((CamDetailActivity) context).typeIndex).giftDTOList;
                ...
                return new CustomHolder<GiftType>(context, lists, itemID){
                    ...
                    public void initView(final int position, List<GiftType> datas, final Context context) {
                        itemView.setOnClickListener(new ClickProxy(new View.OnClickListener() {
                            @Override
                            public void onClick(View v) {
                                ((CamDetailActivity) context).typeIndex = position;
                                notifyTypeData();
                            }
                        }));

                        holderHelper.getView(R.id.item_gift_type_label).setSelected(((CamDetailActivity) context).typeIndex == position);
    ...
    private void notifyTypeData() {
        ...
        data.clear();
        data.addAll(typeData.get(((CamDetailActivity) context).typeIndex).giftDTOList);
        ((CamDetailActivity) context).giftSelectPosition = -1;
    ...
    private void initList(final SendListener sendListener) {
        ...
        list.setAdapter(adapter = new GiftAdapter(context, data, R.layout.item_gift_list, new GiftViewListener(new OnToolsItemClickListener<GiftBean>() {
            ...
            public void onLongClick(int position, GiftBean giftBean, View itemView) {
                ((CamDetailActivity) context).giftSelectPosition = position;
//                getContentView().findViewById(R.id.dialog_gift_list_send).setVisibility(View.VISIBLE);
                count.setText("1");
//                plus.setVisibility(!"0".equals(data.get(((CamDetailActivity) context).giftSelectPosition).ifMoreThanOnce) && !TextUtils.isEmpty(data.get(((CamDetailActivity) context).giftSelectPosition).fiveTimesUrl) ? View.VISIBLE : View.INVISIBLE);
                notifyData();
                ...
                ((CamDetailActivity) context).showGiftPop(location[0] + itemView.getWidth() / 2, location[1], giftBean.giftComboDTOS);
            }
        })));
    }
