        ...
        if(json.length() > 4000) {
            for(int i=0;i<json.length();i+=4000){
                if(i+4000<json.length())
                    Trace.d("http response", i + ((Class<T>)type).getSimpleName() + " ; " + json.substring(i, i+4000));
                else
                    Trace.d("http response", i + ((Class<T>)type).getSimpleName() + " ; " + json.substring(i, json.length()));
            }
        } else {
            Trace.d("http response", ((Class<T>)type).getSimpleName() + " ; " + json);
        }
        String newJson = json.replaceAll(":\"\"", ":null").replace(":''", ":null");
