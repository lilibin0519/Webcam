...
public class PrivateLeftListener extends DefaultAdapterViewListener<PrivateMessage> {

    private final OnToolsItemClickListener<PrivateMessage> listener;

    public PrivateLeftListener(OnToolsItemClickListener<PrivateMessage> listener){
        this.listener = listener;
    }

    @Override
    public CustomHolder getBodyHolder(Context context, List<PrivateMessage> lists, int itemID) {
        PrivateChatLeftHolder holder = new PrivateChatLeftHolder(context, lists);
